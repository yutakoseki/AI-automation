    ...
    <li className={`flex items-center justify-between p-4 bg-white dark:bg-gray-800 rounded-lg shadow ${todo.completed ? 'opacity-50' : ''}`}>
      <label className="flex items-center space-x-3">
        ...
        <div className="flex flex-col">
          ...
          <div aria-label="タグ" role="list" className="flex flex-wrap mt-1">
            {(todo.tags ?? []).map((tag, index) => (
              <span key={index} role="listitem" className="inline-block px-2 py-0.5 text-xs rounded-full bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-200 mr-1 mb-1">
                {tag}
              </span>
            ))}
          </div>
